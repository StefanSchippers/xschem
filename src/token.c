...
const char *tcl_hook2(const char *cmd)
{
  ...
  if(strstr(cmd, "tcleval(") == cmd) {
    ...
    my_strdup2(1530, &result, tclresult());
    /* dbg(0, "tcl_hook2: return: %s\n", result);*/
  } else {
    /* dbg(0, "tcl_hook2: return: %s\n", cmd); */
    my_strdup2(1531, &result, cmd);
  }
  return result;
}
...
int is_generator(const char *name)
{
  ...
  if(!re) {
    re = my_malloc(1533, sizeof(regex_t));
...
char *get_generator_command(const char *str)
{
  ...
  *spc_idx = '\0';
  ...
  #ifdef __unix__
  /* my_strdup(1536, &gen_cmd, cmd_filename); */
  /* add quotes to protect spaces in cmd path */
  my_mstrcat(1537, &gen_cmd, "\"", cmd_filename, "\"", NULL);
  *spc_idx = ' ';
  my_strcat(1538, &gen_cmd, spc_idx);
  #else
  ...
  *spc_idx = ' ';
  ...
  gen_cmd = my_malloc(1539, len * sizeof(char));
...
int set_different_token(char **s,const char *new, const char *old)
{
 ...
 my_realloc(1541, &token, sizetok);
 my_realloc(1542, &value, sizeval);
 ...
 while(1) {
  ...
  if(state==TOK_TOKEN) token[token_pos++]=(char)c;
   ...
   if(strcmp(value, get_tok_value(old,token,1))) {
    mod=1;
    my_strdup(1543, s, subst_token(*s, token, value) );
   }
   state=TOK_BEGIN;
  }
  escape = (c=='\\' && !escape);
 ...
 while(old) {
  ...
  if(state==TOK_TOKEN) token[token_pos++]=(char)c;
   ...
   if(xctx->tok_size == 0 ) {
    mod=1;
    my_strdup(1544, s, subst_token(*s, token, NULL) );
   }
  } else if(state==TOK_END) {
   value[value_pos]='\0';
   value_pos=0;
...
static void get_pin_and_attr(const char *token, char **pin_num_or_name, char **pin_attr)
{
  ...
  while(colon && *p) {
    ...
    if(*p == ':' && !bracket) {
      /*   01234567890123456
       *   @#A[3:0]:net_name */
      *pin_num_or_name = my_malloc(1549, p - token - 1);
      memcpy(*pin_num_or_name, token + 2, p - token - 2);
      (*pin_num_or_name)[p - token - 2] = '\0';
      my_strdup2(1550, pin_attr, p + 1);
      done = 1;
      break;
    }
    if(*p == ']') bracket = 0;
  ...
  if(!done) {
    my_strdup2(1551, pin_num_or_name, token + 2);
    my_strdup2(1552, pin_attr, "");
  }
...
const char *get_tok_value(const char *s,const char *tok, int with_quotes)
{
  ...
  if( size == 0 ) {
    ...
    my_realloc(1556, &result, size);
    my_realloc(1557, &token, sizetok);
  }
  while(1) {
    c=*s++;
    space=SPACE(c) ;
    ...
    if(value_pos>=size) {
      size+=CADCHUNKALLOC;
      my_realloc(1558, &result,size);
    }
    if(token_pos>=sizetok) {
      sizetok+=CADCHUNKALLOC;
      my_realloc(1559, &token,sizetok);
    }
    if(c=='"') {
      if(!escape) quote=!quote;
    }
    if(state==TOK_TOKEN) {
      ...
      if( !cmp ) {
        if(with_quotes & 2) {
          ...
          my_strdup2(1560, &translated_tok, tcl_hook2(result));
...
/* 3: 1+2  :) */
 ...
 my_realloc(1562, &value,sizeval);
 my_realloc(1563, &token,sizetok);
 while(1) {
  c=*s++;
  space=SPACE(c) ;
  dbg(1, "state=%d", state);
...
static char *get_pin_attr_from_inst(int inst, int pin, const char *attr)
{
   ...
   if(str[0]) {
     ...
     my_strdup(1566, &pinname, str);
     pname =my_malloc(1567, tok_val_len + attr_size + 30);
     my_snprintf(pname, tok_val_len + attr_size + 30, "%s(%s)", attr, pinname);
     my_free(1568, &pinname);
     str = get_tok_value(xctx->inst[inst].prop_ptr, pname, 0);
     my_free(1569, &pname);
     if(xctx->tok_size) my_strdup2(1570, &pin_attr_value, str);
     else {
       pnumber = my_malloc(1571, attr_size + 100);
       ...
       my_free(1572, &pnumber);
       if(xctx->tok_size) my_strdup2(1573, &pin_attr_value, str);
     }
   }
...
int get_last_used_index(const char *old_basename, const char *brkt)
{
  ...
  char *refname = my_malloc(1574, size);
...
void hash_names(int inst, int action)
{
  ...
  for(i = start; i < stop; ++i) {
    if(xctx->inst[i].instname && xctx->inst[i].instname[0]) {
      my_strdup(1576, &upinst, expandlabel(xctx->inst[i].instname, &xmult));
      strtoupper(upinst);
...
static int name_is_used(char *name, const char *old_basename, const char *brkt, int q)
{
  ...
  char *upinst_ptr, *upinst_state, *single_name;
  Int_hashentry *entry;
  my_strdup(1578, &upinst, expandlabel(name, &xmult));
  strtoupper(upinst);
  upinst_ptr = upinst;
  while( (single_name = my_strtok_r(upinst_ptr, ",", "", 0, &upinst_state)) ) {
    upinst_ptr = NULL;
...
void new_prop_string(int i, const char *old_prop, int dis_uniq_names)
{
  ...
  if(old_prop==NULL) {
   my_free(1582, &xctx->inst[i].prop_ptr);
   return;
  }
  old_name_len = my_strdup(1583, &old_name,get_tok_value(old_prop,"name",0) ); /* added old_name_len */
 
  if(old_name==NULL) {
   my_strdup(1584, &xctx->inst[i].prop_ptr, old_prop);  /* changed to copy old props if no name */
   my_strdup2(1585, &xctx->inst[i].instname, "");
   return;
  }
  ...
  if(dis_uniq_names || is_used == -1 || is_used == i) {
   my_strdup(1586, &xctx->inst[i].prop_ptr, old_prop);
   my_strdup2(1587, &xctx->inst[i].instname, old_name);
   my_free(1588, &old_name);
   return;
  }
  /* old_name is not unique. Find another unique name */
  old_name_base = my_malloc(1589, old_name_len+1);
  ...
  my_realloc(1590, &new_name, old_name_len + 40);
  ...
  if(strcmp(new_prop, old_prop) ) {
    my_strdup(1592, &xctx->inst[i].prop_ptr, new_prop);
    my_strdup2(1593, &xctx->inst[i].instname, new_name);
  }
  my_free(1594, &old_name);
  my_free(1595, &new_name);
  my_free(1596, &up_new_name);
}
...
static void print_vhdl_primitive(FILE *fd, int inst) /* netlist  primitives, 20071217 */
{
 ...
 my_strdup(1599, &template, (xctx->inst[inst].ptr + xctx->sym)->templ);
 my_strdup(1600, &name, xctx->inst[inst].instname);
 fmt_attr = xctx->format ? xctx->format : "vhdl_format";
 if(!name) my_strdup(1601, &name, get_tok_value(template, "name", 0));
 /* allow format string override in instance */
 my_strdup(1602, &format, get_tok_value(xctx->inst[inst].prop_ptr, fmt_attr, 2));
 /* get netlist format rule from symbol */
 if(!xctx->tok_size)
   my_strdup(1603, &format, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr, fmt_attr, 2));
 /* allow format string override in instance */
 if(xctx->tok_size && strcmp(fmt_attr, "vhdl_format"))               
    my_strdup(1604, &format, get_tok_value(xctx->inst[inst].prop_ptr, "vhdl_format", 2));
 /* get netlist format rule from symbol */
 if(!xctx->tok_size && strcmp(fmt_attr, "vhdl_format"))               
   my_strdup(1605, &format, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr, "vhdl_format", 2));
 if((name==NULL) || (format==NULL) ) {
   my_free(1606, &template);
   my_free(1607, &name);
   my_free(1608, &format);
 ...
 while(1)
 {
  ...
  else if(state==TOK_SEP)                    /* got a token */
  {
   ...
   if(!xctx->tok_size && token[0] =='%') {
     my_mstrcat(1609, &result, token + 1, NULL);
   ...
   {  /* instance names (name) and node labels (lab) go thru the expandlabel function. */
    ...
    if(!(strcmp(token+1,"name"))) {
      if( (lab=expandlabel(value, &tmp)) != NULL)
         my_mstrcat(1610, &result, "----name(", lab, ")", NULL);
      else
         my_mstrcat(1611, &result, value, NULL);
    }
    else if(!(strcmp(token+1,"lab"))) {
      if( (lab=expandlabel(value, &tmp)) != NULL)
         my_mstrcat(1612, &result, "----pin(", lab, ")", NULL);
      else
         my_mstrcat(1613, &result, value, NULL);
    }
    else my_mstrcat(1614, &result, value, NULL);
   }
   else if(strcmp(token,"@symref")==0)
   {
     const char *s = get_sym_name(inst, 9999, 1, 0);
     my_mstrcat(1615, &result, s, NULL);
   }
   ...
   else if(strcmp(token,"@topschname")==0) /* of course topschname must not be present in attributes */
   {
     ...
     my_mstrcat(1620, &result, topsch, NULL);
   }
    ...
    for(i=0;i<no_of_pins; ++i)
    {
      ...
      if(strboolcmp(get_tok_value(prop,"vhdl_ignore",0), "true")) {
        ...
        if(!int_hash_lookup(&table, name, 1, XINSERT_NOREPLACE)) {
          if(!first) my_mstrcat(1621, &result, " , ", NULL);
          str_ptr =  net_name(inst,i, &multip, 0, 1);
          my_mstrcat(1622, &result, "----pin(", str_ptr, ") ", NULL);
          first = 0;
        }
      }
    }
    ...
    for(i=0;i<no_of_pins; ++i) {
     ...
     if(!strcmp( get_tok_value(ptr->rect[PINLAYER][i].prop_ptr,"name",0), token+2)) {
       if(strboolcmp(get_tok_value(ptr->rect[PINLAYER][i].prop_ptr,"vhdl_ignore",0), "true")) {
         str_ptr =  net_name(inst,i, &multip, 0, 1);
         my_mstrcat(1623, &result, "----pin(", str_ptr, ") ", NULL);
       }
       break;
     }
    }
    ...
    * @#0, @#1:net_name, @#2:name, ... */
     ...
     if(n>=0  && pin_attr[0] && n < (xctx->inst[inst].ptr + xctx->sym)->rects[PINLAYER]) {
       ...
       if(!pin_attr_value && is_net_name) {
         ...
         my_strdup(1625, &pin_attr_value,
              xctx->inst[inst].node && xctx->inst[inst].node[n] ? xctx->inst[inst].node[n] : "?");
       }
       if(!pin_attr_value ) my_strdup(1626, &pin_attr_value, "--UNDEF--");
       ...
       if(value[0] && !strcmp(pin_attr, "pinnumber") ) {
         ...
         int slot;
         char *tmpstr = NULL;
         tmpstr = my_malloc(1627, sizeof(xctx->inst[inst].instname));
       ...
       my_mstrcat(1629, &result, value, NULL);
       my_free(1630, &pin_attr_value);
     }
       ...
       if(strboolcmp(si, "true")) {
         ...
         my_mstrcat(1631, &result, "----pin(", str_ptr, ") ", NULL);
       }
     }
     my_free(1632, &pin_attr);
     my_free(1633, &pin_num_or_name);
   }
   ...
   if(c!='%' && c!='@' && c!='\0' ) {
     ...
     my_mstrcat(1637, &result, str, NULL);
   }
   if(c == '@' || c == '%') s--;
   state=TOK_BEGIN;
  }
    ...
    str[0] = (unsigned char) c;
    str[1] = (unsigned char)'\0';
    my_mstrcat(1638, &result, str, NULL);
  }
  ...
  if(c=='\0')
  {
   if(result && strstr(result, "tcleval(")== result) {
     ...
     my_strdup(1639, &result, translate(inst, result));
...
/* return the updated string */
{
  ...
  if(new_val) {
    ...
    if(strcmp(tok, "name") && !is_quoted(new_val) && strpbrk(new_val, ";\n \t")) {
      new_val_copy = my_malloc(1647, new_val_len+3);
      my_snprintf(new_val_copy, new_val_len+3, "\"%s\"", new_val);
    }
    else my_strdup(1648, &new_val_copy, new_val);
  } else new_val_copy = NULL;
  dbg(1, "subst_token(): %s, %s, %s\n", s ? s : "<NULL>", tok ? tok : "<NULL>", new_val ? new_val : "<NULL>");
  sizetok = size = CADCHUNKALLOC;
  my_realloc(1649, &result, size);
  my_realloc(1650, &token, sizetok);
  result[0] = '\0';
  while( s ) {
    c=*s++;
    space=SPACE(c);
...
void print_vhdl_element(FILE *fd, int inst)
{
  ...
  my_strdup(1653, &name,xctx->inst[inst].instname);
  if(!name) my_strdup(1654, &name, get_tok_value(template, "name", 0));
  if(name==NULL) {
    my_free(1655, &name);
    return;
  }
  my_strdup(1656, &template, (xctx->inst[inst].ptr + xctx->sym)->templ);
  no_of_pins= (xctx->inst[inst].ptr + xctx->sym)->rects[PINLAYER];
 ...
 /* print instance name and subckt */
  ...
  my_strdup(1657, &generic_type, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr,"generic_type", 0));
  ...
  /* -------- end print generics passed as properties */
     ...
     for(i=0;i<no_of_generics; ++i)
     {
       ...
       my_strdup(1658, &generic_type,get_tok_value(
         (xctx->inst[inst].ptr + xctx->sym)->rect[GENERICLAYER][i].prop_ptr,"type",0));
       my_strdup(1659, &generic_value,   xctx->inst[inst].node[no_of_pins+i] );
       /*my_strdup(1660, &generic_value, get_tok_value( */
  ...
  my_free(1661, &name);
  my_free(1662, &generic_value);
  my_free(1663, &generic_type);
  my_free(1664, &template);
  my_free(1665, &value);
  my_free(1666, &token);
}
...
void print_generic(FILE *fd, char *ent_or_comp, int symbol)
{
  ...
  my_strdup(1667, &template, xctx->sym[symbol].templ);
  if( !template || !(template[0]) ) {
    my_free(1668, &template);
    return;
  }
  my_strdup(1669, &generic_type, get_tok_value(xctx->sym[symbol].prop_ptr,"generic_type",0));
  ...
  while(1)
  {
   ...
   } else if(state==TOK_END)                    /* got a token */
   {
    ...
    value[value_pos]='\0';
    value_pos=0;
    my_strdup(1670, &type, get_tok_value(generic_type,token,0));
  ...
  if(tmp) fprintf(fd, "\n);\n");
  my_free(1673, &template);
  my_free(1674, &value);
  my_free(1675, &token);
  my_free(1676, &type);
  my_free(1677, &generic_type);
  my_free(1678, &generic_value);
}
...
void print_verilog_param(FILE *fd, int symbol)
{
 ...
 my_strdup(1679, &template, xctx->sym[symbol].templ); /* 20150409 20171103 */
 if( !template || !(template[0]) )  {
   my_free(1680, &template);
   return;
 }
 my_strdup(1681, &generic_type, get_tok_value(xctx->sym[symbol].prop_ptr,"generic_type",0));
 my_strdup(1682, &extra, get_tok_value(xctx->sym[symbol].prop_ptr,"extra",0) );
 ...
 my_free(1683, &template);
 my_free(1684, &generic_type);
 my_free(1685, &value);
 my_free(1686, &token);
 my_free(1687, &extra);
}
...
static int has_included_subcircuit(int inst, int symbol, char **result)
{
  ...
  my_strdup2(1688, &spice_sym_def, get_tok_value(xctx->inst[inst].prop_ptr, "spice_sym_def", 2));
  if(!spice_sym_def[0]) {
    my_strdup2(1689, &spice_sym_def, get_tok_value(xctx->sym[symbol].prop_ptr, "spice_sym_def", 0));
  }
  ...
  if(xctx->tok_size) {
    ...
    my_strdup2(1690, &templ, get_tok_value(xctx->sym[symbol].prop_ptr, "template", 0));
    my_strdup2(1691, &symname, get_tok_value(xctx->inst[inst].prop_ptr, "schematic", 0));
    if(!symname[0]) {
      my_strdup2(1692, &symname, get_tok_value(xctx->sym[symbol].prop_ptr, "schematic", 0));
    }
    if(!symname[0]) {
      my_strdup2(1693, &symname, xctx->sym[symbol].name);
    }
    my_mstrcat(1694, &symname_attr, "symname=", get_cell(symname, 0), NULL);
    my_mstrcat(1695, &symname_attr, " symref=", get_sym_name(inst, 9999, 1, 1), NULL);
    ...
    my_free(1696, &templ);
    my_free(1697, &symname);
    my_free(1698, &symname_attr);
    if(tclresult()[0]) {
      ...
      my_strdup2(1699, &subckt_pinlist, tclresult());
      ...
      for(i = 0;i < no_of_pins; ++i) {
        ...
        if(!spice_ignore) {
          ...
          my_strdup2(1700, &pin_expanded, expandlabel(name, &pin_mult));
          strtolower(pin_expanded);
          my_strdup2(1701, &net_expanded, net_name(inst, i, &net_mult, 0, 1));
          ...
          my_free(1702, &pin_expanded);
          my_free(1703, &net_expanded);
        }
      }
      ...
      while( (subckt_pin = my_strtok_r(subckt_pinlist_ptr, " ", "", 0, &pin_save)) ) {
        ...
        if(entry) {
          ...
          my_mstrcat(1704, &tmp_result, "?1", " ", net, " ", NULL);
        }
      ...
      if(instance_pins == symbol_pins) {
        ret = 1;
        my_mstrcat(1705, result, tmp_result, NULL);
      } else {
      ...
      my_free(1707, &subckt_pinlist);
    }
  }
  my_free(1708, &spice_sym_def);
  return ret;
}
...
void print_spice_subckt_nodes(FILE *fd, int symbol)
{
 ...
 my_strdup(1709, &format1, get_tok_value(xctx->sym[symbol].prop_ptr, fmt_attr, 2));
 if(!xctx->tok_size && strcmp(fmt_attr, "format") )
   my_strdup(1710, &format1, get_tok_value(xctx->sym[symbol].prop_ptr, "format", 2));
 dbg(1, "print_spice_subckt(): format1=%s\n", format1);

 /* can not do this, since @symname is used as a token later in format parser */
 /* my_strdup(1711, &format1,
 ...
 if(format1 && strstr(format1, "tcleval(") == format1) {
    ...
    if(!strcmp(tclres, "?\n")) {
      ...
      *ptr = '\0';
      my_strdup(1712, &format,  format1 + 8);
    } else my_strdup(1713, &format,  tclres);
 } else {
   my_strdup(1714, &format,  format1);
 }
 if(format1) my_free(1715, &format1);
  ...
  else if(state==TOK_SEP)                    /* got a token */
  {
   ...
   if(!strcmp(token, "@spiceprefix")) {
     ...
     for(i=0;i<no_of_pins; ++i)
     {
       if(strboolcmp(get_tok_value(xctx->sym[symbol].rect[PINLAYER][i].prop_ptr,"spice_ignore",0), "true")) {
         ...
         if(!int_hash_lookup(&table, name, 1, XINSERT_NOREPLACE)) {
           ...
           /* fprintf(fd, "%s ", str_ptr); */
           my_mstrcat(1716, &result, str_ptr, " ", NULL);
         }
       }
     }
     int_hash_free(&table);
     ...
     if(i<no_of_pins && strboolcmp(get_tok_value(prop,"spice_ignore",0), "true")) {
       ...
       my_mstrcat(1717, &result, expandlabel(token+2, &multip), " ", NULL);
     }
   }
   /* reference by pin number instead of pin name, allows faster lookup of the attached net name 20180911 */
     ...
     if(pin_number >= 0 && pin_number < no_of_pins) {
       ...
       my_mstrcat(1718, &result, expandlabel(str_ptr, &multip), " ", NULL);
       }
     }
     my_free(1719, &pin_attr);
     my_free(1720, &pin_num_or_name);
    ...
    * in spice_block_netlist(). */
     ...
     if( ( (space  || c == '%' || c == '@') && !escape ) ) {
       ...
       my_mstrcat(1721, &result, token + 1, " ", NULL);
     }
   }
...
int print_spice_element(FILE *fd, int inst)
{
  ...
  size = CADCHUNKALLOC;
  my_realloc(1727, &result, size);
  result[0] = '\0';

  my_strdup(1728, &template, (xctx->inst[inst].ptr + xctx->sym)->templ);
  my_strdup(1729, &name,xctx->inst[inst].instname);
  if (!name) my_strdup(1730, &name, get_tok_value(template, "name", 0));

  fmt_attr = xctx->format ? xctx->format : "format";
  /* allow format string override in instance */
  my_strdup(1731, &format, get_tok_value(xctx->inst[inst].prop_ptr, fmt_attr, 2));
  /* get netlist format rule from symbol */
  if(!xctx->tok_size)
    my_strdup(1732, &format, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr, fmt_attr, 2));
  /* allow format string override in instance */
  if(!xctx->tok_size && strcmp(fmt_attr, "format") )
    my_strdup(1733, &format, get_tok_value(xctx->inst[inst].prop_ptr, "format", 2));
  /* get netlist format rule from symbol */
  if(!xctx->tok_size && strcmp(fmt_attr, "format"))
     my_strdup(1734, &format, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr, "format", 2));
  if ((name==NULL) || (format==NULL)) {
    my_free(1735, &template);
    my_free(1736, &format);
    my_free(1737, &name);
    my_free(1738, &result);
  ...
  while(1)
  {
    ...
    else if (state==TOK_SEP)                    /* got a token */
    {
      ...
      if (!tclgetboolvar("spiceprefix") && !strcmp(token, "@spiceprefix")) {
        ...
        my_strdup2(1739, &val, get_tok_value(xctx->inst[inst].prop_ptr, token+1, 0));
        ...
        if(!strcmp(token, "@spiceprefix") && value[0]) {
          my_realloc(1740, &spiceprefixtag, tok_val_len+22);
      ...
      if(!token_exists && token[0] =='%') {
        ...
        my_mstrcat(1741, &result, token + 1, NULL);
        /* fputs(token + 1, fd); */
      } else if (value && value[0]!='\0') {
         /* instance names (name) and node labels (lab) go thru the expandlabel function. */
        /*if something else must be parsed, put an if here! */
        ...
        if (!(strcmp(token+1,"name") && strcmp(token+1,"lab"))  /* expand name/labels */
          ...
          my_mstrcat(1742, &result, lab, NULL);
          /* fputs(lab,fd); */
          ...
          my_mstrcat(1743, &result, value, NULL);
          /* fputs(value,fd); */
        }
      }
      else if(strcmp(token,"@symref")==0) 
      ...
      else if(strcmp(token,"@topschname")==0) /* of course topschname must not be present in attributes */
      {
        ...
        my_mstrcat(1747, &result, topsch, NULL);
      }
      else if(strcmp(token,"@schname_ext")==0) /* of course schname must not be present in attributes */
      {
        ...
        my_mstrcat(1748, &result, xctx->current_name, NULL);
          ...
          for(i=0;i<no_of_pins; ++i)
          {
            ...
            if(!spice_ignore) {
              if(!int_hash_lookup(&table, name, 1, XINSERT_NOREPLACE)) {
                ...
                my_mstrcat(1751, &result, "?", my_itoa(multip), " ", str_ptr, " ", NULL);
              }
            }
          }
          int_hash_free(&table);
        ...
        for(i=0;i<no_of_pins; ++i) {
          ...
          if (!strcmp( get_tok_value(prop,"name",0), token+2)) {
            if(strboolcmp(get_tok_value(prop,"spice_ignore",0), "true")) {
              ...
              /* result_pos += my_snprintf(result + result_pos, tmp, "?%d %s ", multip, str_ptr); */
              my_mstrcat(1752, &result, "?", my_itoa(multip), " ", str_ptr, " ", NULL);
            }
            break;
          }
        }
       ...
       * @#0, @#1:net_name, @#2:name, ... */
        ...
        if(n>=0  && pin_attr[0] && n < (xctx->inst[inst].ptr + xctx->sym)->rects[PINLAYER]) {
          ...
          if(!pin_attr_value && is_net_name) {
            prepare_netlist_structs(0);
            my_strdup(1754, &pin_attr_value,
                 xctx->inst[inst].node && xctx->inst[inst].node[n] ? xctx->inst[inst].node[n] : "?");
          }
          if(!pin_attr_value ) my_strdup(1755, &pin_attr_value, "--UNDEF--");
          value = pin_attr_value;
          /* recognize slotted devices: instname = "U3:3", value = "a:b:c:d" --> value = "c" */
          if(value[0] && !strcmp(pin_attr, "pinnumber") ) {
            char *ss;
            int slot;
            char *tmpstr = NULL;
            tmpstr = my_malloc(1756, sizeof(xctx->inst[inst].instname));
            if( (ss=strchr(xctx->inst[inst].instname, ':')) ) {
              sscanf(ss+1, "%s", tmpstr);
              if(isonlydigit(tmpstr)) {
                slot = atoi(tmpstr);
          ...
          /* result_pos += my_snprintf(result + result_pos, tmp, "%s", value); */
          my_mstrcat(1758, &result, value, NULL);
          my_free(1759, &pin_attr_value);
        }
        else if(n>=0  && n < (xctx->inst[inst].ptr + xctx->sym)->rects[PINLAYER]) {
          const char *si;
          char *prop = (xctx->inst[inst].ptr + xctx->sym)->rect[PINLAYER][n].prop_ptr;
          ...
          if(strboolcmp(si, "true")) {
            ...
            /* result_pos += my_snprintf(result + result_pos, tmp, "?%d %s ", multip, str_ptr); */
            my_mstrcat(1760, &result, "?", my_itoa(multip), " ", str_ptr, " ", NULL);
          }
        }
        my_free(1761, &pin_attr);
        my_free(1762, &pin_num_or_name);
      }
      else if (!strncmp(token,"@tcleval", 8)) {
        size_t s;
        char *tclcmd=NULL;
        ...
        my_mstrcat(1764, &result, res, NULL);
      ...
      if(c != '%' && c != '@' && c!='\0' ) {
        ...
        my_mstrcat(1766, &result, str, NULL);
        /* fputc(c,fd); */
      }
      if(c == '@' || c == '%' ) s--;
      state=TOK_BEGIN;
      ...
      str[1] = '\0';
      /* result_pos += my_snprintf(result + result_pos, 2, "%c", c); */ /* no realloc needed */
      my_mstrcat(1768, &result, str, NULL);
      /* fputc(c,fd); */
    }
    if(c=='\0')
    {
      ...
      my_mstrcat(1769, &result, str, NULL);
      /* fputc('\n',fd); */
      break;
    }
  } /* while(1) */
  ...
  if(result && strstr(result, "tcleval(")== result) {
    ...
    my_strdup(1770, &result, translate(inst, result));
  ...
  *      if(!*c) break;
  *    }
  *    my_strdup2(1771, &result, translate(inst, result+5));
  *  }
  #endif


  if(result) fprintf(fd, "%s", result);
  my_free(1772, &template);
  my_free(1773, &format);
  my_free(1774, &name);
  my_free(1775, &token);
  my_free(1776, &result);
  my_free(1777, &spiceprefixtag);
  /* my_free(1778, &translatedvalue); */
  return 1;
}
...
void print_tedax_element(FILE *fd, int inst)
{
 ...
 my_strdup(1779, &extra, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr,"extra",0));
 my_strdup(1780, &extra_pinnumber, get_tok_value(xctx->inst[inst].prop_ptr,"extra_pinnumber",0));
 if(!extra_pinnumber) my_strdup(1781, &extra_pinnumber,
         get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr,"extra_pinnumber",0));
 my_strdup(1782, &template,
     (xctx->inst[inst].ptr + xctx->sym)->templ);
 my_strdup(1783, &numslots, get_tok_value(xctx->inst[inst].prop_ptr,"numslots",0));
 if(!numslots) my_strdup(1784, &numslots, get_tok_value(template,"numslots",0));
 if(!numslots) my_strdup(1785, &numslots, "1");

 my_strdup(1786, &name,xctx->inst[inst].instname);
 /* my_strdup(xxx, &name,get_tok_value(xctx->inst[inst].prop_ptr,"name",0)); */
 if(!name) my_strdup(1787, &name, get_tok_value(template, "name", 0));

 /* allow format string override in instance */
 my_strdup(1788, &format, get_tok_value(xctx->inst[inst].prop_ptr,"tedax_format",2));
 if(!format || !format[0])
   my_strdup(1789, &format, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr,"tedax_format",2));
 ...
 if( !format && !strcmp((xctx->inst[inst].ptr + xctx->sym)->type, "subcircuit") ) {
   ...
   int_hash_init(&table, 37);
   for(i=0;i<no_of_pins; ++i) {
     my_strdup2(1790, &net, net_name(inst,i, &net_mult, 0, 1));
     my_strdup2(1791, &pinname, 
       get_tok_value((xctx->inst[inst].ptr + xctx->sym)->rect[PINLAYER][i].prop_ptr,"name",0));
     my_strdup2(1792, &pin, expandlabel(pinname, &pin_mult));
     if(!int_hash_lookup(&table, pinname, 1, XINSERT_NOREPLACE)) {
       ...
       for(n = 0; n < net_mult; ++n) {
         my_strdup(1793, &netbit, find_nth(net, ",", "", 0, n+1));
         my_strdup(1794, &pinbit, find_nth(pin, ",", "", 0, n+1));
 ...
 if(name==NULL || !format || !format[0]) {
   my_free(1800, &extra);
   my_free(1801, &extra_pinnumber);
   my_free(1802, &template);
   my_free(1803, &numslots);
   my_free(1804, &format);
   my_free(1805, &name);
   return;
 }
   ...
   else if(state==TOK_SEP)                   /* got a token */
   {
     ...
     */
      ...
      if(n>=0  && pin_attr[0] && n < (xctx->inst[inst].ptr + xctx->sym)->rects[PINLAYER]) {
        ...
        if(!pin_attr_value && is_net_name) {
          prepare_netlist_structs(0);
          my_strdup(1810, &pin_attr_value,
               xctx->inst[inst].node && xctx->inst[inst].node[n] ? xctx->inst[inst].node[n] : "?");
        }
        if(!pin_attr_value ) my_strdup(1811, &pin_attr_value, "--UNDEF--");
        value = pin_attr_value;
        /* recognize slotted devices: instname = "U3:3", value = "a:b:c:d" --> value = "c" */
        if(value[0] && !strcmp(pin_attr, "pinnumber")) {
          char *ss;
          int slot;
          char *tmpstr = NULL;
          tmpstr = my_malloc(1812, sizeof(xctx->inst[inst].instname));
          if( (ss=strchr(xctx->inst[inst].instname, ':')) ) {
            sscanf(ss+1, "%s", tmpstr);
            if(isonlydigit(tmpstr)) {
              slot = atoi(tmpstr);
      ...
      my_free(1815, &pin_attr);
      my_free(1816, &pin_num_or_name);
    }
    else if(!strncmp(token,"@tcleval", 8)) {
      /* char tclcmd[strlen(token)+100] ; */
      size_t s;
 ...
 my_free(1819, &extra);
 my_free(1820, &extra_pinnumber);
 my_free(1821, &template);
 my_free(1822, &numslots);
 my_free(1823, &format);
 my_free(1824, &name);
 my_free(1825, &token);
}
...
static void print_verilog_primitive(FILE *fd, int inst) /* netlist switch level primitives, 15112003 */
{
  ...
  my_strdup(1826, &template,
      (xctx->inst[inst].ptr + xctx->sym)->templ);

  my_strdup(1827, &name,xctx->inst[inst].instname);
  if(!name) my_strdup(1828, &name, get_tok_value(template, "name", 0));

  fmt_attr = xctx->format ? xctx->format : "verilog_format";
  /* allow format string override in instance */
  my_strdup(1829, &format, get_tok_value(xctx->inst[inst].prop_ptr, fmt_attr, 2));
  /* get netlist format rule from symbol */
  if(!xctx->tok_size)
    my_strdup(1830, &format, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr, fmt_attr, 2));
  /* allow format string override in instance */
  if(!xctx->tok_size && strcmp(fmt_attr, "verilog_format") )
    my_strdup(1831, &format, get_tok_value(xctx->inst[inst].prop_ptr, "verilog_format", 2));
  /* get netlist format rule from symbol */
  if(!xctx->tok_size && strcmp(fmt_attr, "verilog_format"))
     my_strdup(1832, &format, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr, "verilog_format", 2));
  if((name==NULL) || (format==NULL) ) {
    my_free(1833, &template);
    my_free(1834, &name);
    my_free(1835, &format);
    return; /*do no netlist unwanted insts(no format) */
  }
  ...
  while(1)
  {
   ...
   else if(state==TOK_SEP)                    /* got a token */
   {
    ...
    value=get_tok_value(template, token+1, 0);
    if(!xctx->tok_size && token[0] =='%') {
      my_mstrcat(1836, &result, token + 1, NULL);
    } else if(value && value[0]!='\0') {
       /* instance names (name) and node labels (lab) go thru the expandlabel function. */
       /*if something else must be parsed, put an if here! */
     ...
     if(!(strcmp(token+1,"name"))) {
       if( (lab=expandlabel(value, &tmp)) != NULL)
          my_mstrcat(1837, &result, "----name(", lab, ")", NULL);
       else
          my_mstrcat(1838, &result, value, NULL);
     }
     else if(!(strcmp(token+1,"lab"))) {
       if( (lab=expandlabel(value, &tmp)) != NULL)
          my_mstrcat(1839, &result, "----pin(", lab, ")", NULL);
       else
          my_mstrcat(1840, &result, value, NULL);
     }
     else my_mstrcat(1841, &result, value, NULL);
    }
    else if(strcmp(token,"@symref")==0)
    {
      const char *s = get_sym_name(inst, 9999, 1, 0);
      my_mstrcat(1842, &result, s, NULL);
    }
    else if(strcmp(token,"@symname")==0) /* of course symname must not be present  */
                                         /* in hash table */
    {
      const char *s = sanitize(translate(inst, get_sym_name(inst, 0, 0, 0)));
      my_mstrcat(1843, &result, s, NULL);
    }
    else if (strcmp(token,"@symname_ext")==0) 
    {
      const char *s = sanitize(translate(inst, get_sym_name(inst, 0, 1, 0)));
      my_mstrcat(1844, &result, s, NULL);
    }
    else if(strcmp(token,"@schname_ext")==0) /* of course schname must not be present  */
                                         /* in hash table */
    {
      my_mstrcat(1845, &result, xctx->current_name, NULL);
    }
    else if(strcmp(token,"@schname")==0) /* of course schname must not be present  */
                                         /* in hash table */
    {
      my_mstrcat(1846, &result, get_cell(xctx->current_name, 0), NULL);
    }
    else if(strcmp(token,"@topschname")==0) /* of course topschname must not be present in attributes */
    {
      const char *topsch;
      topsch = get_trailing_path(xctx->sch[0], 0, 1);
      my_mstrcat(1847, &result, topsch, NULL);
    }
    else if(strcmp(token,"@pinlist")==0) /* of course pinlist must not be present  */
                                         /* in hash table. print multiplicity */
    {                                    /* and node number: m1 n1 m2 n2 .... */
     ...
     for(i=0;i<no_of_pins; ++i) {
       if(strboolcmp(get_tok_value(xctx->sym[symbol].rect[PINLAYER][i].prop_ptr,"verilog_ignore",0), "true")) {
         const char *name = get_tok_value(xctx->sym[symbol].rect[PINLAYER][i].prop_ptr,"name",0);
         if(!int_hash_lookup(&table, name, 1, XINSERT_NOREPLACE)) {
           if(!first) my_mstrcat(1848, &result, " , ", NULL);
           str_ptr =  net_name(inst,i, &multip, 0, 1);
           my_mstrcat(1849, &result, "----pin(", str_ptr, ") ", NULL);
           first = 0;
         }
       }
     }
     ...
     for(i=0;i<no_of_pins; ++i) {
      ...
      if(!strcmp( get_tok_value(prop,"name",0), token+2)) {
        str_ptr =  net_name(inst,i, &multip, 0, 1);
        my_mstrcat(1850, &result, "----pin(", str_ptr, ") ", NULL);
        break;
      }
     }
    }
     ...
     * @#0, @#1:net_name, @#2:name, ... */
      ...
      if(n>=0  && pin_attr[0] && n < (xctx->inst[inst].ptr + xctx->sym)->rects[PINLAYER]) {
        ...
        if(!pin_attr_value && is_net_name) {
          prepare_netlist_structs(0);
          my_strdup(1852, &pin_attr_value,
               xctx->inst[inst].node && xctx->inst[inst].node[n] ? xctx->inst[inst].node[n] : "?");
        }
        if(!pin_attr_value ) my_strdup(1853, &pin_attr_value, "--UNDEF--");
        value = pin_attr_value;
        /* recognize slotted devices: instname = "U3:3", value = "a:b:c:d" --> value = "c" */
        if(value[0] && !strcmp(pin_attr, "pinnumber") ) {
          char *ss;
          int slot;
          char *tmpstr = NULL;
          tmpstr = my_malloc(1854, sizeof(xctx->inst[inst].instname));
          if( (ss=strchr(xctx->inst[inst].instname, ':')) ) {
            sscanf(ss+1, "%s", tmpstr);
            if(isonlydigit(tmpstr)) {
              slot = atoi(tmpstr);
        ...
        my_mstrcat(1856, &result, value, NULL);
        my_free(1857, &pin_attr_value);
      }
      else if(n>=0  && n < (xctx->inst[inst].ptr + xctx->sym)->rects[PINLAYER]) {
        const char *si;
        char *prop = (xctx->inst[inst].ptr + xctx->sym)->rect[PINLAYER][n].prop_ptr;
        ...
        if(strboolcmp(si, "true")) {
          str_ptr =  net_name(inst,n, &multip, 0, 1);
          my_mstrcat(1858, &result, "----pin(", str_ptr, ") ", NULL);
        }
      }
      my_free(1859, &pin_attr);
      my_free(1860, &pin_num_or_name);
    }
      ...
      char *tclcmd=NULL;
      s = token_pos + strlen(name) + strlen(xctx->inst[inst].name) + 100;
      tclcmd = my_malloc(1861, s);
      Tcl_ResetResult(interp);
      my_snprintf(tclcmd, s, "tclpropeval {%s} {%s} {%s}", token, name, xctx->inst[inst].name);
      tcleval(tclcmd);
      my_mstrcat(1862, &result, tclresult(), NULL);
      my_free(1863, &tclcmd);
    }
    if(c!='%' && c!='@' && c!='\0') {
      char str[2];
      str[0] = (unsigned char) c;
      str[1] = '\0';
      my_mstrcat(1864, &result, str, NULL);
    }
    if(c == '@' || c == '%') s--;
    state=TOK_BEGIN;
   }
     ...
     str[0] = (unsigned char) c;
     str[1] = '\0';
     my_mstrcat(1865, &result, str, NULL);
   }
   if(c=='\0')
   {
    /* do one level of substitutions to resolve @params and equations*/
    if(result && strstr(result, "tcleval(")== result) {
      ...
      my_strdup(1866, &result, translate(inst, result));
...
void print_verilog_element(FILE *fd, int inst)
{
 ...
 my_strdup(1872, &name,xctx->inst[inst].instname);
 if(!name) my_strdup(1873, &name, get_tok_value(template, "name", 0));
 if(name==NULL) {
   my_free(1874, &name);
   return;
 }
 /* verilog_extra is the list of additional nodes passed as attributes */
 my_strdup(1875, &v_extra, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr, "verilog_extra", 0));
 /* extra is the list of attributes NOT to consider as instance parameters */
 my_strdup(1876, &extra, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr, "extra", 0));
 my_strdup(1877, &verilogprefix, 
    get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr, "verilogprefix", 0));
 if(verilogprefix) {
   my_strdup(1878, &symname, verilogprefix);
   my_strcat(1879, &symname, get_sym_name(inst, 0, 0, 0));
 } else {
   my_strdup(1880, &symname, get_sym_name(inst, 0, 0, 0));
 }
 my_free(1881, &verilogprefix);
 my_strdup(1882, &template, (xctx->inst[inst].ptr + xctx->sym)->templ);
 no_of_pins= (xctx->inst[inst].ptr + xctx->sym)->rects[PINLAYER];

 /* 20080915 use generic_type property to decide if some properties are strings, see later */
 my_strdup(1883, &generic_type, get_tok_value((xctx->inst[inst].ptr + xctx->sym)->prop_ptr,"generic_type",0));
 s=xctx->inst[inst].prop_ptr;
/* print instance  subckt */
 dbg(2, "print_verilog_element(): printing inst name & subcircuit name\n");
 fprintf(fd, "%s\n", sanitize(symname));
 my_free(1884, &symname);
 /* -------- print generics passed as properties */
 tmp=0;
 while(1)
 {
...
/* print instance name */
 ...
 my_free(1885, &name);
 my_free(1886, &generic_type);
 my_free(1887, &template);
 my_free(1888, &value);
 my_free(1889, &token);
 my_free(1890, &extra);
 my_free(1891, &v_extra);
}
...
const char *net_name(int i, int j, int *multip, int hash_prefix_unnamed_net, int erc)
{
 ...
 {
   my_strdup(1892, &pinname, get_tok_value( sym->rect[PINLAYER][j].prop_ptr,"name",0));
   ...
   for(k = 0; k < no_of_pins; ++k) {
     ...
     if(!strcmp(duplicated_pinname , pinname)) {
       my_strdup(1893, &pinname, duplicated_pinname);
       j = k;
       break;
     }
   }
 ...
 if(xctx->inst[i].node && xctx->inst[i].node[j] == NULL)
 {
   ...
   if(pinname) my_free(1894, &pinname);
   if(erc) {
   ...
   if(pinname) my_free(1895, &pinname);
...
char *trim_chars(const char *str, const char *sep)
{
  ...
  if(len > result_size) {
    ...
    my_realloc(1897, &result, result_size);
  }
...
char *find_nth(const char *str, const char *sep, const char *quote, int keep_quote, int n)
{
  ...
  if(len > result_size) {
    result_size = len + CADCHUNKALLOC;
    my_realloc(1899, &result, result_size);
  }
...
static char *get_pin_attr(const char *token, int inst, int engineering)
{
  ...
  if(n>=0  && pin_attr[0] && n < (xctx->inst[inst].ptr + xctx->sym)->rects[PINLAYER]) {
    ...
    if(!pin_attr_value && is_net_name) {
      prepare_netlist_structs(0);
      my_strdup2(1901, &pin_attr_value,
           xctx->inst[inst].node && xctx->inst[inst].node[n] ? xctx->inst[inst].node[n] : "");
    }
    else if(!pin_attr_value && !is_net_name && !strcmp(pin_attr, "spice_get_voltage"))
    {
      ...
      if(live && (start_level = sch_waves_loaded()) >= 0 && xctx->raw->annot_p>=0) {
        ...
        if(path) {
          prepare_netlist_structs(0);
          my_strdup2(1902, &net, net_name(inst, n, &multip, 0, 0));
          if(multip == 1 && net && net[0]) {
            ...
            rn = resolved_net(net);
            if(rn) {
              my_strdup2(1903, &fqnet, rn);
              if(rn) my_free(1904, &rn);
              strtolower(fqnet);
              ...
              my_strdup2(1905, &pin_attr_value, valstr);
              dbg(1, "inst %d, net=%s, fqnet=%s idx=%d valstr=%s\n", inst,  net, fqnet, idx, valstr);
              if(fqnet) my_free(1906, &fqnet);
            }
          }
          if(net) my_free(1907, &net);
        }
      }
    }
    ...
    if(!pin_attr_value && !strcmp(pin_attr, "resolved_net")) {
      ...
      my_strdup2(1908, &pin_attr_value, rn ? rn : "");
      if(rn) my_free(1909, &rn);
    }

    if(!pin_attr_value ) my_strdup2(1910, &pin_attr_value, "");
    my_strdup2(1911, &value, pin_attr_value);
    /* recognize slotted devices: instname = "U3:3", value = "a:b:c:d" --> value = "c" */
    if(pin_attr_value[0] && !strcmp(pin_attr, "pinnumber") ) {
      ...
      if( xctx->inst[inst].instname && (ss=strchr(xctx->inst[inst].instname, ':')) ) {
        tmpstr = my_malloc(1912, sizeof(xctx->inst[inst].instname));
        sscanf(ss+1, "%s", tmpstr);
        if(isonlydigit(tmpstr)) {
          slot = atoi(tmpstr);
          if(strstr(value,":")) my_strdup2(1913, &value, find_nth(value, ":", "", 0, slot));
        }
        my_free(1914, &tmpstr);
      }
    }
    my_free(1915, &pin_attr_value);
  }
  /* just @#pin was given */
  else if(n>=0  && n < (xctx->inst[inst].ptr + xctx->sym)->rects[PINLAYER]) {
    const char *str_ptr=NULL;
    ...
    value = my_malloc(1916, tmp);
    my_snprintf(value, tmp, "?%d %s ", multip, str_ptr);
  }
  my_free(1917, &pin_attr);
  my_free(1918, &pin_num_or_name);
...
const char *translate(int inst, const char* s)
{
       ...
       if(no_of_pins == 1) {
         ...
         if(path) {
           ...
           if(xctx->inst[inst].lab) {
             my_strdup2(1923, &net, expandlabel(xctx->inst[inst].lab, &multip));
           }
           if(net == NULL || net[0] == '\0') {
             my_strdup2(1924, &net, net_name(inst, 0, &multip, 0, 0));
           }
           if(multip == 1 && net && net[0]) {
             char *rn;
             ...
             rn = resolved_net(net);
             if(rn) {
               my_strdup2(1925, &fqnet, rn);
               if(rn) my_free(1926, &rn);
               strtolower(fqnet);
               ...
               dbg(1, "inst %d, net=%s, fqnet=%s idx=%d valstr=%s\n", inst,  net, fqnet, idx, valstr);
               if(fqnet) my_free(1927, &fqnet);
             }
           }
           if(net) my_free(1928, &net);
         } 
       }
     }
   }
 ...
 /* if result is like: 'tcleval(some_string)' pass it thru tcl evaluation so expressions
  * can be calculated */
 my_strdup2(1950, &translated_tok, tcl_hook2(result));
 return translated_tok;
}
...
const char *translate2(Lcc *lcc, int level, char* s)
{
  ...
  while (1) {
    ...
    if (state == TOK_TOKEN) token[token_pos++] = (char)c;
      ...
      /* if spiceprefix==0 and token == @spiceprefix then set empty value */
      if(!tclgetboolvar("spiceprefix") && !strcmp(token, "@spiceprefix")) {
        if(value) my_free(1953, &value);
        xctx->tok_size = 0;
      } else if(token[0] == '@' && (token[1] == '@' || token[1] == '#')) { /* get rid of pin attribute info */
        if(value) my_free(1954, &value);
        xctx->tok_size = 0;
      } else {
        my_strdup2(1955, &value, get_tok_value(lcc[level].prop_ptr, token + 1, 0));
        /* propagate %xxx tokens to upper levels if no value found */
        if(!value[0]  && token[0] == '%') {
          my_strdup2(1956, &value, token + 1);
      ...
      if(xctx->tok_size && value[0]) {
        ...
        while(i > 1) {
          ...
          if(xctx->tok_size && upperval[0]) {
            my_strdup2(1957, &value, upperval);
          } else {
            break;
          }
          i--;
...
const char *translate3(const char *s, int eat_escapes, const char *s1, const char *s2, const char *s3)
{
 ...
 if(!s || !xctx) {
   my_free(1964, &result);
   my_free(1965, &translated_tok);
   return empty;
 }
 dbg(2, "translate3():\n   s=%s\n   s1=%s\n   s2=%s\n   s3=%s\n", s, s1, s2, s3);
 my_strdup2(1966, &result, "");
 ...
 while(1) {
  ...
  if(state==TOK_TOKEN) token[token_pos++]=(char)c;
   ...
   if(!xctx->tok_size) { /* above lines did not find a value for token */
     ...
     my_strcat(1967, &result, token);
   } else {
     my_strdup2(1968, &value1, value);
     my_strcat(1969, &result, value1);
     my_free(1970, &value1);
   }
   token_pos = 0;
   ...
   ch[0] = (char)c;
   ch[1] = '\0';
   my_strcat(1972, &result, ch);
  }
  if(c=='\0') {
   /* my_strcat(1973, &result, ""); */
   break;
  }
 } /* while(1) */
 ...
 my_strdup2(1975, &translated_tok, tcl_hook2(result));
 return translated_tok;
}
