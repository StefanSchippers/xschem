...
void attach_labels_to_inst(int interactive) /*  offloaded from callback.c 20171005 */
{
  ...
  my_strdup(74, &symname_pin, tcleval("find_file_first lab_pin.sym"));
  my_strdup(75, &symname_wire, tcleval("find_file_first lab_wire.sym"));
...
/*  use the given params, otherwise query user */
{
 ...
 if(i!=-1)
 {
  ...
  if(xctx->sym[i].type && !strcmp(xctx->sym[i].type, "scope")) {
    ...
    my_strdup(90, &xctx->inst[n].prop_ptr,
          subst_token(xctx->inst[n].prop_ptr, "attach", xctx->inst[n].instname));
    my_mstrcat(91, &prop, "name=", xctx->inst[n].instname, "\n", NULL);
    my_mstrcat(92, &prop, "flags=graph,unlocked\n", NULL);
    my_mstrcat(93, &prop, "lock=1\n", NULL);
    my_mstrcat(94, &prop, "color=8\n", NULL);
    if(xctx->sym[i].rects[PINLAYER] == 0) {
      if(xctx->lastsel == 1 && xctx->sel_array[0].type==ELEMENT) {
        my_mstrcat(95, &prop, "node=\"tcleval([xschem get_fqdevice [xschem translate ",
                                       xctx->inst[n].instname, " @device]])\"\n", NULL);
        my_strdup(96, &xctx->inst[n].prop_ptr,
        ...
        #if 1
        if(xctx->inst[n].instname) my_free(97, &xctx->inst[n].instname);       
        if(xctx->inst[n].name) my_free(98, &xctx->inst[n].name);       
        if(xctx->inst[n].prop_ptr) my_free(99, &xctx->inst[n].prop_ptr);       
        if(xctx->inst[n].lab) my_free(100, &xctx->inst[n].lab);       
        if(prop) my_free(101, &prop);
        xctx->instances--;
        return 0;
        #endif
      }
...
int copy_hierarchy_data(const char *from_win_path, const char *to_win_path)
{
  ...
  for(i = 0; i <= from->currsch; i++) {
    my_strdup2(105, &to->sch[i], sch[i]);
    my_strdup2(106, &to->sch_path[i], sch_path[i]);
    ...
    my_strdup2(107, &to->hier_attr[i].prop_ptr, hier_attr[i].prop_ptr);
    my_strdup2(108, &to->hier_attr[i].templ, hier_attr[i].templ);
    my_strdup2(109, &to->hier_attr[i].symname, hier_attr[i].symname);
    ...
    for(j = 0; j < HASHSIZE; j++) {
      ...
      while(*fromnext) {
        Str_hashentry *e;
        e = my_calloc(110, 1, sizeof(Str_hashentry));
        e->hash = (*fromnext)->hash;
        my_strdup2(111, &e->token, (*fromnext)->token);
        my_strdup2(112, &e->value, (*fromnext)->value);
        *tonext = e;
...
void launcher(void)
{
  ...
  if(xctx->lastsel ==1)
  {
    ...
    my_strdup2(113, &command, get_tok_value(prop_ptr,"tclcommand",0));
...
void copy_symbol(xSymbol *dest_sym, xSymbol *src_sym)
{
  ...
  my_strdup2(115, &dest_sym->name, src_sym->name);
  my_strdup2(116, &dest_sym->type, src_sym->type);
  my_strdup2(117, &dest_sym->templ, src_sym->templ);
  my_strdup(118, &dest_sym->parent_prop_ptr, src_sym->parent_prop_ptr);
  my_strdup2(119, &dest_sym->prop_ptr, src_sym->prop_ptr);

  dest_sym->line = my_calloc(120, cadlayers, sizeof(xLine *));
  dest_sym->poly = my_calloc(121, cadlayers, sizeof(xPoly *));
  dest_sym->arc = my_calloc(122, cadlayers, sizeof(xArc *));
  dest_sym->rect = my_calloc(123, cadlayers, sizeof(xRect *));
  dest_sym->lines = my_calloc(124, cadlayers, sizeof(int));
  dest_sym->rects = my_calloc(125, cadlayers, sizeof(int));
  dest_sym->arcs = my_calloc(126, cadlayers, sizeof(int));
  dest_sym->polygons = my_calloc(127, cadlayers, sizeof(int));

  dest_sym->text = my_calloc(128, src_sym->texts, sizeof(xText));
  ...
  for(c = 0;c<cadlayers; ++c) {
    /* symbol lines */
    dest_sym->line[c] = my_calloc(129, src_sym->lines[c], sizeof(xLine));
    for(j = 0; j < src_sym->lines[c]; ++j) { 
      dest_sym->line[c][j] = src_sym->line[c][j];
      dest_sym->line[c][j].prop_ptr = NULL;
      my_strdup(130, &dest_sym->line[c][j].prop_ptr, src_sym->line[c][j].prop_ptr);
    }
    /* symbol rects */
    dest_sym->rect[c] = my_calloc(131, src_sym->rects[c], sizeof(xRect));
    for(j = 0; j < src_sym->rects[c]; ++j) { 
      ...
      my_strdup(132, &dest_sym->rect[c][j].prop_ptr, src_sym->rect[c][j].prop_ptr);
    }
    /* symbol arcs */
    dest_sym->arc[c] = my_calloc(133, src_sym->arcs[c], sizeof(xArc));
    for(j = 0; j < src_sym->arcs[c]; ++j) { 
      dest_sym->arc[c][j] = src_sym->arc[c][j];
      dest_sym->arc[c][j].prop_ptr = NULL;
      my_strdup(134, &dest_sym->arc[c][j].prop_ptr, src_sym->arc[c][j].prop_ptr);
    }
    /* symbol polygons */
    dest_sym->poly[c] = my_calloc(135, src_sym->polygons[c], sizeof(xPoly));
    for(j = 0; j < src_sym->polygons[c]; ++j) { 
      ...
      dest_sym->poly[c][j].x = my_malloc(136, points * sizeof(double));
      dest_sym->poly[c][j].y = my_malloc(137, points * sizeof(double));
      dest_sym->poly[c][j].selected_point = my_malloc(138, points * sizeof(unsigned short));
      my_strdup(139, &dest_sym->poly[c][j].prop_ptr, src_sym->poly[c][j].prop_ptr);
  ...
  for(j = 0; j < src_sym->texts; ++j) {
    ...
    my_strdup2(140, &dest_sym->text[j].prop_ptr, src_sym->text[j].prop_ptr);
    my_strdup2(141, &dest_sym->text[j].floater_ptr, src_sym->text[j].floater_ptr);
    dbg(1, "copy_symbol1(): allocating sym %d text %d\n", dest_sym - xctx->sym, j);
    my_strdup2(142, &dest_sym->text[j].txt_ptr, src_sym->text[j].txt_ptr);
    my_strdup2(143, &dest_sym->text[j].font, src_sym->text[j].font);
    my_strdup2(144, &dest_sym->text[j].floater_instname, src_sym->text[j].floater_instname);
  }
}   
  ...
  if(attr) {
    ...
    for(n=0; n < xctx->lastsel; ++n) {
      if(xctx->sel_array[n].type == ELEMENT) {
        ...
        if(flag == 1) {
          my_strdup(145, &xctx->inst[i].prop_ptr, subst_token(xctx->inst[i].prop_ptr, attr, "true"));
        } else if(flag == 2) {
          my_strdup(146, &xctx->inst[i].prop_ptr, subst_token(xctx->inst[i].prop_ptr, attr, "short"));
        } else {
          my_strdup(147, &xctx->inst[i].prop_ptr, subst_token(xctx->inst[i].prop_ptr, attr, NULL));
        }
        set_inst_flags(&xctx->inst[i]);
        set_modify(1);
      }
...
void get_additional_symbols(int what)
{
  ...
  if(what == 1) { /* start */
    ...
    for(i=0;i<xctx->instances; ++i) {
      ...
      my_strdup(148, &spice_sym_def, get_tok_value(xctx->inst[i].prop_ptr,"spice_sym_def",6));
      my_strdup(149, &verilog_sym_def, get_tok_value(xctx->inst[i].prop_ptr,"verilog_sym_def",4));
      my_strdup(150, &vhdl_sym_def, get_tok_value(xctx->inst[i].prop_ptr,"vhdl_sym_def",4));

      /* resolve schematic=generator.tcl( @n ) where n=11 is defined in instance attrs */
      my_strdup2(151, &sch, get_tok_value(xctx->inst[i].prop_ptr,"schematic", 6));
      dbg(1, "get_additional_symbols(): schematic=%s\n", sch);
      schematic_token_found = xctx->tok_size;
      my_strdup2(152, &sch, translate3(sch, 1, xctx->inst[i].prop_ptr, NULL, NULL, NULL));
      dbg(1, "get_additional_symbols(): sch=%s tok_size= %ld\n", sch, xctx->tok_size);
    
      my_strdup2(153, &sch, tcl_hook2(
      ...
      if(schematic_token_found && sch[0]) { /* `schematic` token exists  and a schematic is specified */
        ...
        my_strdup2(154, &default_schematic, get_tok_value(symptr->prop_ptr,"default_schematic",0));
        ...
        if(is_gen) {
          my_strdup2(155, &sym, sch);
          dbg(1, "get_additional_symbols(): generator\n");
        } else {
          my_strdup2(156, &sym, add_ext(rel_sym_path(sch), ".sym"));
        }

        my_mstrcat(157, &symname_attr, "symname=", get_cell(sym, 0), NULL);
        my_mstrcat(158, &symname_attr, " symref=", get_sym_name(i, 9999, 1, 1), NULL);
        my_strdup(159, &spice_sym_def, 
            translate3(spice_sym_def, 1, xctx->inst[i].prop_ptr,
                                         symptr->templ, 
                                         symname_attr, NULL));
        dbg(1, "get_additional_symbols(): spice_sym_def=%s\n", spice_sym_def);
        my_free(160, &symname_attr);
        ...
        if(!found) {
          ...
          my_strdup(161, &xctx->sym[j].name, sym);

          my_strdup(162, &xctx->sym[j].parent_prop_ptr, xctx->inst[i].prop_ptr);
          /* the copied symbol will not inherit the default_schematic attribute otherwise it will also
           * be skipped */
          if(default_schematic) {
            my_strdup(163, &xctx->sym[j].prop_ptr, 
        ...
        my_free(168, &sym);
        my_free(169, &default_schematic);
      } /* if(xctx->tok_size && sch[0]) */
      my_free(170, &sch);
      my_free(171, &spice_sym_def);
      my_free(172, &vhdl_sym_def);
      my_free(173, &verilog_sym_def);
    } /* for(i=0;i<xctx->instances; ++i) */
...
void get_sch_from_sym(char *filename, xSymbol *sym, int inst, int fallback)
{
  ...
  /* resolve schematic=generator.tcl( @n ) where n=11 is defined in instance attrs */
  if(inst >=0 ) {
    my_strdup(174, &str_tmp, translate3(get_tok_value(xctx->inst[inst].prop_ptr,"schematic", 6),
              1, xctx->inst[inst].prop_ptr, NULL, NULL, NULL));
  }
  if(!str_tmp) my_strdup2(175, &str_tmp,  get_tok_value(sym->prop_ptr, "schematic", 6));
  if(str_tmp[0]) { /* schematic attribute in symbol or instance was given */
    ...
    my_strdup2(176, &sch, tcl_hook2(str_replace(str_tmp, "@symname",
...
int change_sch_path(int instnumber, int dr)
{
  ...
  int res = 0;
  if(level < 0 ) return 0;
  my_strdup2(179, &instname, get_tok_value(xctx->hier_attr[level].prop_ptr, "name", 0));
  my_strdup2(180, &expanded_instname, expandlabel(instname, &inst_mult));
  my_strdup2(181, &path, xctx->sch_path[xctx->currsch]);
  ...
  if(!ptr) goto end;
  *(ptr+1) = '\0';
  my_free(182, &xctx->sch_path[xctx->currsch]);
  my_strcat(183, &xctx->sch_path[xctx->currsch], path);
  my_strcat(184, &xctx->sch_path[xctx->currsch], find_nth(expanded_instname, ",", "", 0, instnumber));
  my_strcat(185, &xctx->sch_path[xctx->currsch], ".");
  xctx->sch_path_hash[xctx->currsch] = 0;
  ...
  my_free(186, &instname);
  my_free(187, &path);
  my_free(188, &expanded_instname);
  return res;
}
...
int descend_schematic(int instnumber, int fallback, int alert, int set_title)
{
 ...
 if(xctx->lastsel !=1 || xctx->sel_array[0].type!=ELEMENT) {
   ...
   if(xctx->inst[n].instname && xctx->inst[n].instname[0]) {
     if(set_title & 4)  {
       my_strdup2(189, &str, xctx->inst[n].instname);
       inst_mult = 1;
       instnumber = 1;
     } else {
       my_strdup2(190, &str, expandlabel(xctx->inst[n].instname, &inst_mult));
     }
   } else {
     my_strdup2(191, &str, "");
     inst_mult = 1;
   ...
   my_strdup(193, &xctx->sch_path[xctx->currsch+1], xctx->sch_path[xctx->currsch]);
   ...
   if(!(set_title & 2)) for(i = 0; i < xctx->sym[xctx->inst[n].ptr].rects[PINLAYER]; i++) {
     ...
     my_strdup2(194, &pin_node, expandlabel(pin_name, &mult));
     my_strdup2(195, &net_node, expandlabel(xctx->inst[n].node[i], &net_mult));

     p_n_s1 = pin_node;
     for(k = 1; k<=mult; ++k) {
         single_p = my_strtok_r(p_n_s1, ",", "", 0, &p_n_s2);
         p_n_s1 = NULL;
         my_strdup2(196, &single_n,
             find_nth(net_node, ",", "", 0, ((inst_number - 1) * mult + k - 1) % net_mult + 1));
         single_n_ptr = single_n;
         if(single_n_ptr[0] == '#') {
           if(mult > 1) {
             my_mstrcat(197, &single_n, "[", my_itoa((inst_mult - inst_number + 1) * mult - k), "]", NULL);
           }
     ...
     if(single_n) my_free(198, &single_n);
     my_free(199, &net_node);
     my_free(200, &pin_node);
   }

   my_strdup(201, &xctx->hier_attr[xctx->currsch].prop_ptr, 
             xctx->inst[n].prop_ptr);
   my_strdup(202, &xctx->hier_attr[xctx->currsch].templ, xctx->sym[xctx->inst[n].ptr].templ);

   dbg(1,"descend_schematic(): inst_number=%d\n", inst_number);
   my_strcat(203, &xctx->sch_path[xctx->currsch+1], find_nth(str, ",", "", 0, inst_number));
   my_free(204, &str);
   dbg(1,"descend_schematic(): inst_number=%d\n", inst_number);
   my_strcat(205, &xctx->sch_path[xctx->currsch+1], ".");
...
void go_back(int what)
{
 ...
 if(xctx->currsch>0)
 {
  ...
  my_free(206, &xctx->sch[xctx->currsch]);
  if(xctx->portmap[xctx->currsch].table) str_hash_free(&xctx->portmap[xctx->currsch]);

  xctx->sch_path_hash[xctx->currsch] = 0;
  xctx->currsch--;
  my_free(207, &xctx->hier_attr[xctx->currsch].prop_ptr);
  my_free(208, &xctx->hier_attr[xctx->currsch].templ);
...
void clear_schematic(int cancel, int symbol)
{
      ...
      if(cancel != -1) { /* -1 means user cancel save request */
        ...
        if(symbol == 1) {
          ...
          my_free(209, &xctx->sch[xctx->currsch]);
          my_mstrcat(210, &xctx->sch[xctx->currsch], pwd_dir, "/", name, NULL);
          ...
          my_free(211, &xctx->sch[xctx->currsch]);
          my_mstrcat(212, &xctx->sch[xctx->currsch], pwd_dir, "/", name, NULL);
          my_strncpy(xctx->current_name, name, S(xctx->current_name));
        }
        draw();
        set_modify(0);
   ...
   for(k=0;k<xctx->lastsel; ++k)
   {
     ...
     if(type==POLYGON && xctx->poly[c][n].sel==SELECTED) {
       ...
       if(xctx->rectcolor != xctx->text[n].layer) {
         char *p;
         my_strdup2(214, &xctx->text[n].prop_ptr, 
...
void new_polygon(int what, double mousex_snap, double mousey_snap)
{
   ...
   if(xctx->nl_points >= xctx->nl_maxpoints-1) {  /*  check storage for 2 xctx->nl_points */
     ...
     my_realloc(215, &xctx->nl_polyx, sizeof(double)*xctx->nl_maxpoints);
     my_realloc(216, &xctx->nl_polyy, sizeof(double)*xctx->nl_maxpoints);
   }
   if( what & PLACE )
   {
   ...
   if((what & SET || (what & END)) ||
     ...
     my_free(217, &xctx->nl_polyx);
     my_free(218, &xctx->nl_polyy);
    ...
    const char *props, double hsize, double vsize)
{
  ...
  t->xscale= hsize;
  t->yscale= vsize;
  my_strdup(222, &t->prop_ptr, props);
  /*  debug ... */
  /*  t->prop_ptr=NULL; */
