...
{
  if(!hilight_hash_copy(xctx, get_old_xctx())) xctx->hilight_nets = 1;
}

int hilight_hash_copy(Xschem_ctx *dest, Xschem_ctx *source)
{
  int i, res = 1; /* 1 = error or nothing found */
  if (!source || !dest) return 1; /* nothing done */
  if (!source->hilight_table || !dest->hilight_table) return 1; /* nothing done */
  for(i = 0; i < HASHSIZE; ++i) {
    Hilight_hashentry *sentry = source->hilight_table[i];
    ...
    while(sentry) {
      ...
      new_node->oldvalue = sentry->oldvalue;
      new_node->value = sentry->value;
      new_node->time = sentry->time;
      new_node->next = NULL;
      *dentry = new_node;
      dentry = &new_node->next;
      sentry = sentry->next;
      res = 0; /* something has been found -> Success */
    }
  }
  return res;
}
